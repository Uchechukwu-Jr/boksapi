...
app.post("/api/books", (req, res) => {
    try {
        ...
        if (newBook.series) {
            ...
            newBook.series.numberInSeries =
                newBook.series.numberInSeries.trim();
        }

        newBook.id = uuidv4();
        Books.push(newBook);
        fs.writeFileSync(
            "./db/BooksReverse.json",
...
app.get("/api/series", (req, res) => {
    ...
    const seriesBooks = Books.filter(
        book => book.series && book.series.seriesTitle
    );
...
const requestedSeries = req.params.series.toLowerCase().split(" ");

    // Filter books by the provided series title
    const seriesBooks = Books.filter(book => {
        if (!book.series) return false;
        
        const seriesTitleWords = book.series.seriesTitle.toLowerCase().split(" ");
        
        // Check if all words in the requested series title are present in the book's series title
        return requestedSeries.every(word => seriesTitleWords.includes(word));
    });

    if (seriesBooks.length === 0) {
        return res.status(404).json({ error: 'No books found in the specified series.' });
    }

    // Sort books by the number in the series
    seriesBooks.sort((a, b) => {
        const numA = parseInt(a.series.numberInSeries);
        const numB = parseInt(b.series.numberInSeries);
        return numA - numB;
    });
